...
#i have copied the code from the reference get-network-devices.py and modifying it

#Since we can import any file containing python code
...
d=dict()
for index in # import requests library
import requests
...
d=dict()
for i in r_resp["response"][0]:
	d.values()
	d[i["Deviceid]=d.get["deviceid"]+d.get["macaddress"]]
	d.values(deviceid)


def gethostcount(n):
	count=0
	if devices not in hosts:
		devices==1
	else:
		devices+=1
